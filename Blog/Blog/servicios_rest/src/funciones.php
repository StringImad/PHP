...
function obtener_categorias()
{
    try {
        ...
        try {
            ...
            $respuesta["categorias"] = $sentencia->fetchAll(PDO::FETCH_ASSOC);
...
function borrar_noticia($id)
{
    try {
        $conexion = new PDO("mysql:host=" . SERVIDOR_BD . ";dbname=" . NOMBRE_BD, USUARIO_BD, CLAVE_BD, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'utf8'"));
        try {
            $consulta = "DELETE noticias, comentarios
            FROM noticias
            LEFT JOIN comentarios ON noticias.idNoticia = comentarios.idNoticia
            ...
            $respuesta["mensaje"] = "El noticia ha sido borrado correctamente";
...
function modificar_noticia($datos)
{
    try {
        ...
        try {
            ...
            $consulta = "update noticias set titulo=?, copete=?, cuerpo=?, idCategoria=? where idNoticia=?";


            $sentencia = $conexion->prepare($consulta);
            $sentencia->execute($datos);

            $respuesta["mensaje"] = "Se ha modificado correctamente la noticia datos 0: ".$datos[0]. " -1: ".$datos[1]." -2 ".$datos[2]." -3: ".$datos[3]." -4: ".$datos[4] ;
